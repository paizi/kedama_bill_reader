...
if not os.path.exists(cache_path+ '\\logs.txt'):
    print('输入 .minecraft 目录下的 logs 文件夹路径，如：D:\Game\MultiMC\instances\\1.18.2_YASMP_v1.0.220308\.minecraft\logs\n只有第一次要输')
    logs_path = input('logs 文件夹路径:')
...
while True:
    ...
    with open(latest_log, 'r') as f:
        for line in f:
            if "您收到了" in line:
                logs_list.append(str(now)[0:10] + ' ' + line[0:11] + line[39:-1])
